...
def scrape_search():
    ...
    try:
        time.sleep(random.uniform(1.5, 3.5))  # avoid bot detection
...
def get_product_status(url):
    try:
        ...
        # Availability - more robust checks
        avail = "Unknown"
        ...
        if avail_section:
            ...
            # Fallback text search in whole page
        ...
        print(f"Product error for {url}: {e}")
        return {'title': "Error", 'availability': "Error", 'price': "N/A"}

# ====================== TASKS (now crash-proof) ======================
@bot.event
async def on_ready():
